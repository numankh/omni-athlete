#!/usr/bin/env python3
"""
Show total running miles per month over time with a chart.
"""
...
def main():
    ...
    # Query: total running miles per month
    query = """
    SELECT
    ...
    if not rows:
        print("No running workouts found with distances.")
        return
    ...
    plt.plot(months, miles, marker='o', label='Monthly Miles')
    plt.plot(months, y_fit, '--', color='red', label='Trendline')
    plt.title('Running Miles Per Month')
    plt.xlabel('Month')
